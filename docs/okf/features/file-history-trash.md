...
履歴とTrashは、そのファイルが現在の**Workspace**内にあるか、それとも**Files directory**内にあるかによって保存場所（scope）が分かれます。[WorkspaceとFilesディレクトリ](/features/workspace-files.md)の設定次第でファイルの所属先が変わるため、後からWorkspace直下へファイルを移動した場合、その前後で履歴の保存先が変わり、片方のscopeで探しても見つからないことがあります。どちらにも属さない場所のファイルには、履歴が記録されません。
...
* 64 MiBを超えるファイルは、履歴作成のために巨大な内容をmemoryへ読み込まないようcheckpointの対象外です。別媒体でバックアップしてください。
