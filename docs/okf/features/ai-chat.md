...
`Settings > 音声入力`で、従来のブラウザ認識（初期値）とOpenAI互換を切り替えられます。OpenAI互換の「サービス」でOpenAI、whisper.cpp、その他を選べます。OpenAIを選ぶとBase URLに`https://api.openai.com/v1`、Modelに`whisper-1`が自動入力されます。音声入力の設定はAI設定から独立しています。
マイクの確認と、無音データを使った接続テストもここで実行できます。
「送信の合図」はブラウザ認識・OpenAI互換・whisper.cppで利用できます。初期値は`over, オーバー`です。任意の言葉をカンマ区切りで設定でき、空欄なら自動送信しません。
...
文字起こしをキャンセルした録音は同じChat内で保持されます。次に録音した音声と順につなげて、まとめて文字起こしします。「保持中の録音を変換」では追加録音なしで再変換できます。変換成功後に保持音声を自動で消去します。
