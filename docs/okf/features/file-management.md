...
拡張子が対応形式か、ファイルがFiles directory内にあるか、OSの読み取り権限があるかを確認します。symlinkの参照先がFiles directory外なら安全のため拒否されます。外部エディタが起動しない場合は、設定した値がショートカットではなく実行ファイル本体のpathか確認してください。
