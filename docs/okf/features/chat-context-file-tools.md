...
API providerでは、設定したfile tool modeに応じてWorking directory内のファイルを読み、検索し、作成・編集・renameできます。変更を伴う操作はpending actionとして提示され、ユーザーの確認後にだけ適用されます。Working directory外へのpath traversalやsymlink escapeは拒否されます。
