export async function executeWorkflowScript(code: string, variables: Record<string, string | number>, timeoutMs = 10_000): Promise<unknown> {
  const source = `
self.onmessage = async (event) => {
  try {
    ...
    const run = new AsyncFunction("variables", "\\\"use strict\\\"; const window=undefined, document=undefined, localStorage=undefined, indexedDB=undefined, navigator=undefined;\\n" + event.data.code);
  ...
  try {
    return await new Promise((resolve, reject) => {
      const timer = window.setTimeout(() => { worker.terminate(); reject(new Error(`Script timed out after ${timeoutMs} ms`)); }, timeoutMs);
