...
import { readProjectStateFile, writeProjectStateFile } from "../lib/wailsBackend";
...
function scopeKey(projectBase: string): string {
  return projectBase || "__session__";
}

function notify(projectBase: string, records: WorkflowRun[]): void {
  window.dispatchEvent(new CustomEvent("llm-hub:workflow-history-changed", { detail: { projectBase, records } }));
}

async function persistHistory(records: WorkflowRun[], projectBase: string): Promise<void> {
  const key = scopeKey(projectBase);
  cache.set(key, records);
  notify(projectBase, records);
  ...
  const queued = previous.catch(() => undefined).then(async () => {
    ...
    await writeProjectStateFile(workflowHistoryStateFile, content);
  });
...
export async function loadWorkflowHistory(projectBase = "", force = false): Promise<WorkflowRun[]> {
  const key = scopeKey(projectBase);
  if (!force && cache.has(key)) return cache.get(key)!;
  try {
    const raw = await readProjectStateFile(workflowHistoryStateFile);
    if (!raw) {
      cache.set(key, []);
      return [];
    }
...
export async function appendWorkflowHistory(run: WorkflowRun, projectBase = ""): Promise<WorkflowRun[]> {
  return mutateHistory(projectBase, (stored) => {
...
export async function clearWorkflowHistory(workflowPath: string | undefined, projectBase = ""): Promise<WorkflowRun[]> {
  return mutateHistory(projectBase, (stored) => workflowPath ? stored.filter((item) => item.workflowPath !== workflowPath) : []);
}

export async function removeWorkflowHistory(id: string, projectBase = ""): Promise<WorkflowRun[]> {
  return mutateHistory(projectBase, (stored) => stored.filter((item) => item.id !== id));
}

function mutateHistory(projectBase: string, change: (records: WorkflowRun[]) => WorkflowRun[]): Promise<WorkflowRun[]> {
  const key = scopeKey(projectBase);
  const previous = mutationQueues.get(key) ?? Promise.resolve(cache.get(key) ?? []);
  const task = previous.catch(() => []).then(async () => {
    const stored = cache.has(key) ? cache.get(key)! : await loadWorkflowHistory(projectBase);
    const next = change(stored);
    await persistHistory(next, projectBase);
    return next;
  });
