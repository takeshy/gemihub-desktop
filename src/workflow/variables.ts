...
function normalizeLegacyName(name: string): string {
  return name.startsWith("__") && name.endsWith("__") && name.length > 4 ? `_${name.slice(2, -2)}` : name;
}

function legacyName(name: string): string {
  return name.startsWith("_") && !name.startsWith("__") && name.length > 1 ? `__${name.slice(1)}__` : name;
}

export function getWorkflowVariable(variables: WorkflowVariables, name: string): string | number | undefined {
  const direct = variables.get(name);
  if (direct !== undefined) return direct;
  const normalized = normalizeLegacyName(name);
  if (normalized !== name && variables.has(normalized)) return variables.get(normalized);
  const legacy = legacyName(name);
  return legacy !== name ? variables.get(legacy) : undefined;
}

function nestedValue(value: unknown, path: string[], variables: WorkflowVariables): unknown {
  let current = value;
...
export function replaceWorkflowVariables(template: string, variables: WorkflowVariables): string {
  let result = template, previous = "";
  for (let iteration = 0; iteration < 10 && result !== previous; iteration++) {
    previous = result;
    result = result.replace(/\{\{([\w.[\]]+)(:json)?\}\}/g, (original, fullPath: string, jsonMode?: string) => {
      ...
      if (rest.length && typeof value === "string") {
        try {
          ...
          value = nestedValue(JSON.parse(fenced?.[1].trim() ?? value), rest, variables);
...
export function evaluateWorkflowCondition(expression: string, variables: WorkflowVariables): boolean {
  ...
  const leftNumber = Number.parseFloat(left), rightNumber = Number.parseFloat(right);
  const numeric = !Number.isNaN(leftNumber) && !Number.isNaN(rightNumber);
  switch (match[2]) {
    case "==": return numeric ? leftNumber === rightNumber : left === right;
    case "!=": return numeric ? leftNumber !== rightNumber : left !== right;
    case "contains": { try { const value = JSON.parse(left); if (Array.isArray(value)) return value.includes(right); } catch { /* use string containment */ } return left.includes(right); }
    case "<": return numeric ? leftNumber < rightNumber : left < right;
    case ">": return numeric ? leftNumber > rightNumber : left > right;
    case "<=": return numeric ? leftNumber <= rightNumber : left <= right;
    case ">=": return numeric ? leftNumber >= rightNumber : left >= right;
...
export function evaluateWorkflowValue(expression: string, variables: WorkflowVariables): string | number {
  const replaced = replaceWorkflowVariables(expression, variables);
  const arithmetic = replaced.match(/^(-?\d+(?:\.\d+)?)\s*([+*/%-])\s*(-?\d+(?:\.\d+)?)$/);
  if (arithmetic) {
