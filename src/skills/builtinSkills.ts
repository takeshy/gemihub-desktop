...
const JSON_CANVAS: BuiltinSkillDefinition = {
  id: "json-canvas",
  name: "json-canvas",
  description: "Create and edit JSON Canvas files (.canvas) with nodes, edges, groups, and connections. Use when working with .canvas files or creating visual canvases, mind maps, and flowcharts.",
...
const BASE: BuiltinSkillDefinition = {
  id: "base",
  name: "base",
  description: "Create and edit Bases (.base files) with views, filters, formulas, and summaries. Use when working with .base files or creating database-like table, card, and list views of workspace files.",
  instructions: `# Bases Skill
...
const DASHBOARD: BuiltinSkillDefinition = {
  id: "dashboard",
  name: "dashboard",
  description: "Create GemiHub Desktop dashboards (.dashboard files): a grid of widgets that embed Bases views, files, memos, workflows, and web pages. Use when the user asks for a dashboard, home page, overview, or a grid of workspace resources.",
  instructions: `# Dashboard Skill

Create \`.dashboard\` files for GemiHub Desktop: a grid of widgets that embed
**Bases** views, workspace files, memos, workflows, and web pages. Opening a \`.dashboard\`
...
rendered as Markdown; text files are editable; HTML, images, and PDFs
...
export const DEFAULT_BUILTIN_SKILL_IDS = ["markdown"];
...
export function contextualBuiltinFolderPath(path: string | undefined): string | null {
  ...
  if (lower.endsWith(".base")) return builtinFolderPath("base");
  if (lower.endsWith(".dashboard")) return builtinFolderPath("dashboard");
  return null;
}
...
export function getBuiltinSkillMetadata(): WorkspaceSkill[] {
...
export function loadBuiltinSkill(folderPath: string): WorkspaceSkill | null {
