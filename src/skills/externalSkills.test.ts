...
import { compareVersions, fetchSkillCatalog, getSafeSkillTargetPath, installSkillFiles } from "./externalSkills.ts";
...
Deno.test("external skill semver and paths follow the catalog contract", () => {
  ...
  assertEquals(getSafeSkillTargetPath("review", "review/SKILL.md"), "skills/review/SKILL.md");
  ...
  runtime.window = { go: { main: { App: {
    ExternalHTTPRequest: (request: { url: string }) => {
      const body = request.url.endsWith("/takeshy/llm-hub-skills")
        ...
        ? JSON.stringify({ tree: [{ type: "blob", path: "skills/okf/manifest.json" }] })
        : JSON.stringify({ id: "okf", name: "OKF", version: "0.1.2", description: "Author OKF", compatibility: { plugins: [{ id: "gemihub", minVersion: "0.24.0" }] } });
      return Promise.resolve({ status: 200, headers: {}, body, bodyBase64: "" });
    },
  } } } };
  try { assertEquals(await fetchSkillCatalog(), [{ id: "okf", name: "OKF", version: "0.1.2", description: "Author OKF" }]); }
...
Deno.test("external skills apply workspace host patches before installation", async () => {
  ...
  try {
    const manifest = JSON.stringify({ id: "patched", version: "1.0.0", compatibility: { plugins: [{ id: "llm-hub-workspace" }] }, hostPatches: { "llm-hub-workspace": ["patches/workspace.patch"] } });
    ...
    assertEquals(writes.find(([path]) => path === "skills/patched/SKILL.md")?.[1], "workspace\n");
  } finally {
...
Deno.test("external skill installer validates manifests and confines writes", async () => {
  ...
  target.go = { main: { App: {
    ...
    WriteFile: (path: string, content: string) => { writes.set(path, content); return Promise.resolve(); },
  } } };
  try {
    const result = await installSkillFiles([
      { relativePath: "review/SKILL.md", content: "---\nname: Review\n---\n" },
      { relativePath: "review/manifest.json", content: JSON.stringify({ id: "review", name: "Review", version: "1.0.0", compatibility: { plugins: [{ id: "llm-hub" }] } }) },
      { relativePath: "review/references/checklist.md", content: "Check tests" },
    ], ["review"]);
    assertEquals(result.installed, ["review"]);
    assertEquals([...writes.keys()].sort(), ["skills/review/SKILL.md", "skills/review/manifest.json", "skills/review/references/checklist.md"]);

    const unsafe = await installSkillFiles([
      { relativePath: "bad/SKILL.md", content: "skill" },
      { relativePath: "bad/manifest.json", content: JSON.stringify({ id: "bad", version: "1.0.0" }) },
  ...
  } finally { target.go = previous; runtime.window = previousWindow; }
});
