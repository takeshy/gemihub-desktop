import { createDirectory, externalHTTPRequest, listWorkspaceFiles, readFile, writeFile } from "../lib/wailsBackend";
...
export async function listInstalledSkills(): Promise<InstalledSkill[]> {
  const skillFiles = (await listWorkspaceFiles()).filter((file) => /^skills\/[^/]+\/SKILL\.md$/i.test(file.path));
