...
import { externalHTTPRequest, installPluginFiles, readFile, uninstallManagedPlugin } from "../lib/wailsBackend";
import type { PluginConfig, PluginManifest, PluginPermission } from "./types";

export const PLUGIN_HOST_ID = "gemihub-desktop";
export const PLUGIN_HOST_VERSION = "0.10.0";
const LEGACY_PLUGIN_HOST_ID = "llm-hub-workspace";

const SEMVER_RE = /^(\d+)\.(\d+)\.(\d+)(?:-([0-9A-Za-z.-]+))?(?:\+[0-9A-Za-z.-]+)?$/;
const KNOWN_PERMISSIONS = new Set<PluginPermission>(["files", "storage", "network", "llm", "drive", "gemini", "calendar", "gmail", "sheets"]);

interface ReleaseAsset { name: string; browser_download_url: string; size?: number }
interface GitHubRelease { tag_name: string; assets: ReleaseAsset[] }
...
function parseSemver(value: string): [number, number, number, string[]] | null {
  ...
  return match ? [Number(match[1]), Number(match[2]), Number(match[3]), match[4]?.split(".") ?? []] : null;
}
...
export function comparePluginVersions(leftValue: string, rightValue: string): number | null {
  const left = parseSemver(leftValue), right = parseSemver(rightValue);
  if (!left || !right) return null;
  return left[0] - right[0] || left[1] - right[1] || left[2] - right[2] || comparePrerelease(left[3], right[3]);
}
...
async function getJSON<T>(url: string): Promise<T> {
  const response = await externalHTTPRequest({ url, method: "GET", headers: { Accept: "application/vnd.github+json" } });
  if (response.status < 200 || response.status >= 300) throw new Error(`Request failed: HTTP ${response.status}`);
  return JSON.parse(response.body) as T;
}

async function getText(url: string): Promise<string> {
  const response = await externalHTTPRequest({ url, method: "GET", headers: {} });
...
function validateManifest(raw: unknown, releaseTag: string): PluginManifest {
  if (!raw || typeof raw !== "object" || Array.isArray(raw)) throw new Error("Invalid manifest.json");
  const manifest = raw as PluginManifest;
  for (const field of ["id", "name", "version", "minAppVersion", "description", "author"] as const) {
    if (typeof manifest[field] !== "string" || !manifest[field]?.trim()) throw new Error(`manifest.json requires ${field}`);
  }
  if (!/^[A-Za-z0-9][A-Za-z0-9._-]*$/.test(manifest.id)) throw new Error("Invalid plugin id");
  if (!parseSemver(manifest.version) || !parseSemver(manifest.minAppVersion!)) throw new Error("Plugin versions must use semver");
  if (comparePluginVersions(manifest.version, releaseTag) !== 0) throw new Error("Release tag and manifest version do not match");
  if ((comparePluginVersions(PLUGIN_HOST_VERSION, manifest.minAppVersion!) ?? -1) < 0) throw new Error(`Plugin requires ${PLUGIN_HOST_ID} ${manifest.minAppVersion} or newer`);
  if (manifest.permissions && (!Array.isArray(manifest.permissions) || manifest.permissions.some((permission) => !KNOWN_PERMISSIONS.has(permission)))) throw new Error("manifest.json contains an unknown permission");
  if (manifest.assets) {
    if (!Array.isArray(manifest.assets)) throw new Error("manifest assets must be an array");
    const names = new Set<string>();
    for (const asset of manifest.assets) {
      if (!asset || !/^[A-Za-z0-9][A-Za-z0-9._-]*$/.test(asset.name) || names.has(asset.name)) throw new Error("Invalid or duplicate external asset name");
      names.add(asset.name);
      let parsed: URL;
      try { parsed = new URL(asset.url); } catch { throw new Error(`Invalid asset URL: ${asset.name}`); }
      if (parsed.protocol !== "https:") throw new Error(`Asset URL must use HTTPS: ${asset.name}`);
...
async function releaseInfo(repo: string): Promise<{ release: GitHubRelease; manifest: PluginManifest; manifestText: string }> {
  ...
  if (!manifestAsset || !release.assets.some((asset) => asset.name === "main.js")) throw new Error("Release requires manifest.json and main.js assets");
  const manifestText = await getText(manifestAsset.browser_download_url);
  let raw: unknown;
  try { raw = JSON.parse(manifestText); } catch { throw new Error("Invalid manifest.json"); }
  return { release, manifest: validateManifest(raw, release.tag_name), manifestText };
}
...
export async function installPluginRelease(input: string, expectedID?: string, approvedPreview?: PluginPreview): Promise<{ config: PluginConfig; metadata: PluginInstallMetadata }> {
  ...
  const sourceFiles = [{ relativePath: `${manifest.id}/manifest.json`, content: manifestText }, ...downloaded];
  const patched = applyHostPatches(manifest.id, sourceFiles, manifest, patchHostID, { protectedPaths: ["manifest.json"] });
  ...
  const files = Object.fromEntries(patched.files.filter((file) => !patchSet.has(file.relativePath)).map((file) => [file.relativePath.slice(manifest.id.length + 1), file.content]));
  if (!files["main.js"]) throw new Error("Host patch removed main.js");
  const metadata: PluginInstallMetadata = {
    id: manifest.id, repo, version: manifest.version, releaseTag: release.tag_name,
    host: PLUGIN_HOST_ID, installedAt: new Date().toISOString(),
    patches: await Promise.all(patched.applied.map(async (name) => ({
      name,
      sha256: await sha256Text(downloaded.find((file) => file.relativePath === `${manifest.id}/${name}`)?.content ?? ""),
    }))),
  };
  await installPluginFiles(manifest.id, files, JSON.stringify(metadata, null, 2));
  return { config: { id: manifest.id, enabled: true, version: manifest.version, releaseTag: release.tag_name, repo, source: "github", permissions: manifest.permissions }, metadata };
}

export async function readPluginInstallMetadata(id: string): Promise<PluginInstallMetadata | null> {
  try {
    const file = await readFile(`.llm-hub/plugins/${id}/install.json`);
    return file?.content ? JSON.parse(file.content) as PluginInstallMetadata : null;
  } catch { return null; }
}

export async function checkPluginUpdate(config: PluginConfig): Promise<PluginPreview | null> {
  if (!config.repo) return null;
  const preview = await previewPluginRelease(config.repo);
  const comparison = comparePluginVersions(preview.manifest.version, config.version);
  if (comparison === null) throw new Error("Installed plugin has an invalid version");
