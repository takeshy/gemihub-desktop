...
import {
  loadInstalledAgentPlugin,
  mergeAgentPluginMcpServer,
} from "../agentPlugins/manager";
import {
  checkPluginUpdate,
...
function workspaceConfigKey(workspaceBase: string): string {
  return `${CONFIG_KEY}:${encodeURIComponent(workspaceBase)}`;
}

function workspaceSelectedPluginKey(workspaceBase: string): string {
  return `${SELECTED_PLUGIN_KEY}:${encodeURIComponent(workspaceBase)}`;
}

function storedConfigs(key: string): PluginConfig[] {
  try {
    return JSON.parse(localStorage.getItem(key) || "[]") as PluginConfig[];
  } catch {
    return [];
  }
}

function PluginMainViewWidget(
  { view, api, language, config }: {
    view: PluginView;
    api: PluginAPI;
    language: string;
    config: unknown;
...
export function PluginHost({
  ...
  const selectedPluginKey = useMemo(
    () => workspaceSelectedPluginKey(workspaceBase),
    [workspaceBase],
  );
  const [configs, setConfigs] = useState<PluginConfig[]>(() =>
    storedConfigs(configKey)
  );
  ...
  useEffect(() => {
    if (chatOpenRequest > 0) setActiveTab("chat");
  }, [chatOpenRequest]);
  const [selectedPluginId, setSelectedPluginId] = useState(() =>
    localStorage.getItem(selectedPluginKey) || ""
  );
  const [settingsPluginId, setSettingsPluginId] = useState("");
  const [settingsContainer, setSettingsContainer] = useState<
    HTMLElement | null
  >(null);
  const [chatAttachmentRequest, setChatAttachmentRequest] = useState<
    {
      id: number;
    ...
    void (async () => {
      ...
      const settled = await Promise.allSettled(
        enabled.map(async (item) => ({
          name: item.name,
          package: await loadInstalledAgentPlugin(item.name, workspaceBase),
        })),
      );
      if (cancelled) return;
      const loaded = new Map(
        settled.flatMap((result) =>
          result.status === "fulfilled"
            ? [[result.value.name, result.value.package] as const]
            : []
        ),
      );
      const installedNames = new Set(installed.map((item) => item.name));
      onChatSettingsChange((current) => {
        const unmanaged = current.mcpServers.filter((server) =>
          !server.agentPlugin
        );
        ...
        for (const server of current.mcpServers) {
          if (
            server.agentPlugin &&
            installedNames.has(server.agentPlugin.pluginName)
          ) {
            previousByPlugin.set(server.agentPlugin.pluginName, [
              ...(previousByPlugin.get(server.agentPlugin.pluginName) ?? []),
              server,
            ]);
          }
        }
        const managed = installed.flatMap((plugin) => {
          ...
          if (!plugin.enabled) {
            return previous.map((server) => ({ ...server, enabled: false }));
          }
          const contents = loaded.get(plugin.name);
          if (!contents) return previous;
          const byName = new Map(
            previous.map((server) => [server.agentPlugin!.serverName, server]),
          );
          return contents.mcpServers.map((server) =>
            mergeAgentPluginMcpServer(
              server,
              byName.get(server.agentPlugin!.serverName),
            )
          );
        });
        const next = [...unmanaged, ...managed];
        return JSON.stringify(next) === JSON.stringify(current.mcpServers)
          ? current
          : { ...current, mcpServers: next };
      });
    })().catch((error) => {
      if (!cancelled) {
        console.error("Failed to synchronize Agent Plugins:", error);
      }
    });
    return () => {
      cancelled = true;
  ...
  useEffect(() => {
    setSettingsContainer(
      settingsOpen ? document.getElementById("plugin-settings-manager") : null,
    );
  }, [settingsOpen]);
  ...
  useEffect(() => {
    ...
    if (selectedPluginId) {
      localStorage.setItem(selectedPluginKey, selectedPluginId);
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      const loaded = await Promise.all(ids.map(async (id) => {
        try {
          return {
            manifest: await readPluginManifest(id),
            install: await readPluginInstallMetadata(id),
          };
        } catch {
          return null;
        }
      ...
      if (!cancelled) {
        const valid = loaded.filter((item): item is NonNullable<typeof item> =>
          !!item
        );
        ...
        setConfigs((current) => {
          ...
          for (const item of valid) {
            ...
            const existing = next.find((config) =>
              config.id === item.manifest.id
            );
            const managed: PluginConfig = {
              ...
              releaseTag: item.install.releaseTag,
              permissions: item.manifest.permissions,
            };
            next = existing
              ? next.map((config) =>
                config.id === managed.id ? managed : config
              )
              : [...next, managed];
          }
          return next;
  ...
  useEffect(() => {
    ...
    const callbacks = {
      ...
      onRegisterSettingsTab: (tab: PluginSettingsTab) => {
        registeredSettings.push(tab);
        if (!cancelled) setSettingsTabs([...registeredSettings]);
      },
      onRegisterSlashCommand: (command: PluginSlashCommand) => {
        registeredCommands.push(command);
        if (!cancelled) setSlashCommands([...registeredCommands]);
      },
      onLLMListModels: async () =>
        configuredModelOptions(chatSettings).map((option) => ({
          id: option.key,
          label: option.label,
      ...
      onLLMChat: async (
        ...
        const knownModelId = configuredModelOptions(chatSettings).some((
          option,
        ) => option.key === modelId);
        ...
        const result = await chat({
          ...
          model: selectedSettings.provider === "cli"
            ? ""
            : selectedSettings.model,
          vertexProjectId: selectedSettings.vertexProjectId,
          vertexLocation: selectedSettings.vertexLocation,
          systemPrompt: options?.systemPrompt || selectedSettings.systemPrompt,
          ...
          fileToolMode: "none",
          cliType: selectedSettings.cliType,
          cliPath: selectedSettings.cliPaths[selectedSettings.cliType],
          cliSessionId: "",
        });
      ...
      for (const manifest of manifests) {
        ...
        try {
          ...
          if (!cancelled) {
            setErrors((current) => ({
              ...current,
              [manifest.id]: error instanceof Error
                ? error.message
                : String(error),
            }));
          }
  ...
  const pluginChoices = useMemo(() => {
    ...
    return ids.map((pluginId) => ({
      ...
      name: manifests.find((manifest) => manifest.id === pluginId)?.name ||
        views.find((view) =>
          view.pluginId === pluginId && view.location === "sidebar"
        )?.name ||
        views.find((view) =>
          view.pluginId === pluginId && view.location === "main"
        )?.name ||
        pluginId,
    }));
  }, [manifests, views]);
  const activePluginId =
    pluginChoices.some((choice) => choice.pluginId === selectedPluginId)
      ? selectedPluginId
      : pluginChoices[0]?.pluginId || "";
  const activeView = sidebarViews.find((view) =>
    view.pluginId === activePluginId
  );
  const activeApi = activePluginId
    ? apiMapRef.current.get(activePluginId)
    : null;
  const ActiveViewComponent = activeView?.component;
  const activePluginMainView = mainViews.find((view) =>
    view.pluginId === activePluginId
  );
  const activeFileMatchesPlugin = !!activeFile?.path &&
    !!activePluginMainView &&
    (!activePluginMainView.extensions?.length ||
      !!pluginViewForPath([activePluginMainView], activeFile.path));
  ...
  const activatePlugin = (pluginId: string) => {
    ...
    if (mainView) {
      ...
      const matchesActiveFile = !mainView.extensions?.length ||
        mainView.extensions.some((extension) =>
          activePath.toLowerCase().endsWith(extension.toLowerCase())
        );
  ...
  const installFromGitHub = async () => {
    ...
    try {
      ...
      if (
        existing &&
        (existing.source !== "github" || existing.repo !== preview.repo)
      ) {
        throw new Error(
          `Plugin id ${preview.manifest.id} is already installed from another source.`,
        );
      }
      if (
        !window.confirm(
          `Install ${preview.manifest.name} ${preview.manifest.version}?\nDeclared API permissions: ${permissions}\nWorkspace patches: ${patches}${
            recommendation === "third-party"
              ? "\n\nNot recommended: this is neither an official nor a locally developed plugin."
              : ""
          }\n\nThe plugin will be installed disabled.`,
        )
      ) return;
      ...
      setConfigs((
        ...
        ...current.filter((config) => config.id !== installed.config.id),
        installed.config,
      ]);
      setRepoInput("");
      setManagerMessage(
        `Installed ${preview.manifest.name} ${preview.manifest.version}.`,
      );
      setPluginRefresh((value) => value + 1);
    } catch (error) {
      setManagerMessage(error instanceof Error ? error.message : String(error));
    } finally {
      setPluginBusy(null);
    }
  };
  ...
  const updateFromGitHub = async (config: PluginConfig) => {
    ...
    setPluginBusy(config.id);
    setManagerMessage("");
    try {
      const preview = await checkPluginUpdate(config);
      if (!preview) {
        setManagerMessage(`${config.id} is up to date.`);
        return;
      }
      const oldPermissions = new Set(config.permissions ?? []);
      const added = (preview.manifest.permissions ?? []).filter((permission) =>
      ...
      if (
        !window.confirm(
          `Update ${preview.manifest.name} from ${config.version} to ${preview.manifest.version}?${detail}`,
        )
      ) return;
      const installed = await installPluginRelease(
        config.repo,
      ...
      setConfigs((current) =>
        current.map((item) =>
          item.id === config.id ? { ...installed.config, enabled: false } : item
        )
      );
      setManagerMessage(
        `Updated ${preview.manifest.name} to ${preview.manifest.version}.`,
      );
      setPluginRefresh((value) => value + 1);
    } catch (error) {
      setManagerMessage(error instanceof Error ? error.message : String(error));
  ...
  const uninstallGitHubPlugin = async (config: PluginConfig) => {
    if (
      !window.confirm(
        `Uninstall ${config.id} and its cached assets and plugin data?`,
      )
    ) return;
    setPluginBusy(config.id);
    setManagerMessage("");
        ...
        <section className="plugin-manager settings-plugin-manager">
          <header>
            <div>
              ...
              <small>GemiHub-compatible Workspace extensions</small>
            </div>
          </header>
          {!workspaceBase && (
            <p>Select a Workspace directory to discover `.llm-hub/plugins`.</p>
          )}
          {workspaceBase && (
            <form
            ...
            >
              <input
                ...
                aria-label="GitHub plugin repository"
                disabled={!!pluginBusy}
              />
              <button
                type="submit"
                disabled={!repoInput.trim() || !!pluginBusy}
              >
                {pluginBusy === "install"
          ...
          {workspaceBase && (
            <AgentPluginsSection
              workspaceBase={workspaceBase}
              mcpServers={chatSettings.mcpServers}
              onChanged={() => setAgentPluginRefresh((value) => value + 1)}
              onMcpServersTested={(pluginName, servers) =>
                onChatSettingsChange((current) => ({
                  ...current,
                  mcpServers: [
                    ...current.mcpServers.filter((server) =>
                      server.agentPlugin?.pluginName !== pluginName
                    ),
                    ...servers,
                  ],
                }))}
          ...
          {manifests.map((manifest) => {
            ...
            const recommendation = pluginRecommendation(config);
            const hasView = pluginChoices.some((choice) =>
              choice.pluginId === manifest.id
            );
            const settingsTab = settingsTabs.find((tab) =>
              tab.pluginId === manifest.id
            );
            ...
            return (
              <article
                key={manifest.id}
                className={settingsPluginId === manifest.id
                  ? "plugin-manager-selected"
                  : ""}
              >
                <button
                  type="button"
                  className="plugin-manager-open"
                  disabled={!enabled || !hasView}
                  onClick={() => openPluginView(manifest.id)}
                >
                  <span>
                    <strong>{manifest.name}</strong>
                    <small>
                      {manifest.description || manifest.id} · {manifest.version}
                    </small>
                    <small
                      className={`plugin-recommendation plugin-recommendation-${recommendation}`}
                    >
                      {recommendation === "official"
                        ? "Official"
                        : recommendation === "custom"
                        ? "Custom / locally managed"
                        : "Third-party · Not recommended"}
                    </small>
                  </span>
                  {hasView && <ChevronRight size={16} />}
                </button>
                <label className="plugin-toggle">
                  <input
                    type="checkbox"
                    checked={enabled}
                    onChange={() => togglePlugin(manifest)}
                    title={enabled ? "Disable plugin" : "Enable plugin"}
                  />
                  <span />
                </label>
                {(managed || SettingsComponent) && (
                    ...
                    {managed && (
                      <>
                        <button
                          ...
                          disabled={!!pluginBusy}
                          onClick={() => void updateFromGitHub(config)}
                        >
                          {pluginBusy === config.id
                        ...
                        <button
                          ...
                          title="Uninstall"
                          disabled={!!pluginBusy}
                          onClick={() => void uninstallGitHubPlugin(config)}
                        >
                          <Trash2 size={13} />
                        </button>
                      </>
                ...
                {settingsPluginId === manifest.id && SettingsComponent &&
                  settingsApi && (
                  <div className="plugin-manager-settings">
                    <SettingsComponent api={settingsApi} language={language} />
                  </div>
                )}
              </article>
            );
