...
function mockRuntime(manifest = pluginManifest()) {
  const installed: Array<{ id: string; files: Record<string, string>; metadata: string }> = [];
  const assets = [
    { name: "manifest.json", browser_download_url: "https://downloads.example/manifest.json" },
    { name: "main.js", browser_download_url: "https://downloads.example/main.js" },
    { name: "workspace.patch", browser_download_url: "https://downloads.example/workspace.patch" },
  ];
  const patch = "--- a/main.js\n+++ b/main.js\n@@ -1,1 +1,1 @@\n-original\n+workspace\n";
  const backend = {
    ExternalHTTPRequest: ({ url }: { url: string }) => Promise.resolve({
      status: 200, headers: {}, bodyBase64: "", body: url.includes("/releases/latest")
        ? JSON.stringify({ tag_name: "v0.1.0", assets })
        : url.endsWith("manifest.json") ? manifest
        : url.endsWith("workspace.patch") ? patch : "original\n",
    }),
    InstallPluginFiles: (id: string, files: Record<string, string>, metadata: string) => { installed.push({ id, files, metadata }); return Promise.resolve(); },
  };
...
Deno.test("plugin release rejects incompatible hosts and tag mismatches", async () => {
  ...
  const previous = runtime.window;
  try {
    const incompatible = mockRuntime(pluginManifest({ minAppVersion: "9.0.0" }));
    runtime.window = { go: { main: { App: incompatible.backend } } };
    const hostError = await assertRejects(() => previewPluginRelease("owner/repo"));
    assertStringIncludes(hostError instanceof Error ? hostError.message : String(hostError), "requires");
