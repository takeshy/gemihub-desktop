...
function layout(value: unknown, fallback: LayoutPos): LayoutPos {
  ...
  return {
    ...
    w: Math.max(1, number(item.w, fallback.w)),
    h: Math.max(1, number(item.h, fallback.h)),
  };
}
function grid(value: unknown): DashboardGrid {
  ...
  const item = value as Record<string, unknown>;
  return {
    cols: Math.max(1, number(item.cols, 12)),
    rowHeight: Math.max(20, number(item.rowHeight, 80)),
    gap: Math.max(0, number(item.gap, 8)),
  };
}

export function dashboardName(path: string): string {
  const base = path.replaceAll("\\", "/").split("/").pop() || "Dashboard";
  return base.toLowerCase().endsWith(DASHBOARD_EXT)
  ...
  try {
    ...
    const widgets: DashboardWidget[] = Array.isArray(root.widgets)
      ? root.widgets.flatMap((raw, index) => {
        ...
        const rawLayout = record.layout && typeof record.layout === "object" &&
        ...
        const sm = !looksFlat && rawLayout.sm
          ? layout(rawLayout.sm, {
            x: 0,
            y: index * lg.h,
            w: parsedGrid.cols,
...
export function serializeDashboard(data: DashboardData): string {
  const widgets = data.widgets.map((widget) => {
    ...
    return {
      ...stored,
      layout: { ...(layoutBreakpoints || {}), lg: widget.layout },
    };
  });
...
export function deriveSmallLayout(data: DashboardData): DashboardData {
  ...
  const ordered = [...data.widgets].sort((left, right) =>
  ...
  for (const widget of ordered) {
    ...
    if (existing) {
      positions.set(widget.id, existing);
      y = Math.max(y, existing.y + existing.h);
    } else {
      const next = { x: 0, y, w: cols, h: widget.layout.h };
      positions.set(widget.id, next);
      y += next.h;
    }
  }
  return {
...
export function widgetLabel(type: string): string {
  ...
  } as Record<string, string>)[type] || `Unknown (${type})`;
}

export async function listDashboardFiles(): Promise<DashboardFileEntry[]> {
  return (await listWorkspaceFiles()).filter((entry) =>
    entry.path.toLowerCase().endsWith(DASHBOARD_EXT) &&
    !/(?:^|\/)(?:trash|history)\//i.test(entry.path)
  )
    .map((entry) => ({
      path: entry.path,
      name: dashboardName(entry.path),
...
export async function createDashboard(
  name: string,
): Promise<{ path: string; data: DashboardData }> {
