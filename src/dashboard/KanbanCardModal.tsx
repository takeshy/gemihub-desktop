...
function clampGeometry(value: ModalGeometry): ModalGeometry {
  ...
  const width = Math.min(
    value.width,
    Math.max(320, window.innerWidth - margin * 2),
  );
  ...
  return {
    ...
    height,
    left: Math.min(
      Math.max(margin, value.left),
      Math.max(margin, window.innerWidth - width - margin),
    ),
    top: Math.min(
      Math.max(margin, value.top),
...
export function KanbanCardModal(
  ...
  const [geometry, setGeometry] = useState<ModalGeometry>(() =>
    clampGeometry(sessionGeometry ?? initialGeometry())
  );
  const interaction = useRef<
    | { kind: "move" | "resize"; x: number; y: number; initial: ModalGeometry }
    | null
  >(null);
    ...
    const move = (event: PointerEvent) => {
      ...
      if (active.kind === "move") {
        setGeometry(
          clampGeometry({
            ...active.initial,
            left: active.initial.left + dx,
            top: active.initial.top + dy,
          }),
        );
      } else {
        setGeometry(clampGeometry({
          ...
          width: Math.max(
            ...
            active.initial.width + dx,
          ),
          height: Math.max(
            Math.min(320, window.innerHeight - 16),
            active.initial.height + dy,
          ),
        }));
      }
    };
  ...
  const changeMode = (next: CardMode) => {
    sessionMode = next;
    setMode(next);
  };
  const save = async () => {
    ...
    >
      ...
      >
        ...
        <div className="kanban-card-modal-body">
          {loading
            ? <div className="dashboard-widget-empty">Loading…</div>
            : error && !content
        ...
        <button
          type="button"
          className="kanban-card-modal-resize"
