...
function shouldCollapse(body: string): boolean {
  ...
  return body.length > COLLAPSE_CHAR_LIMIT ||
    lines.length > COLLAPSE_LINE_LIMIT;
}
...
function collapsedBody(body: string): string {
  ...
  const byLines = lines.length > COLLAPSE_LINE_LIMIT
    ...
    : body;
  const clipped = byLines.length <= COLLAPSE_CHAR_LIMIT
    ? byLines
    : byLines.slice(0, COLLAPSE_CHAR_LIMIT);
  return `${clipped.trimEnd()}\n\n...`;
}

export function memoHoverPreview(entry: MemoEntry): string {
  const text = (entry.body || entry.quote).replace(/\s+/g, " ").trim();
  return text.length > HOVER_PREVIEW_LIMIT
    ? `${text.slice(0, HOVER_PREVIEW_LIMIT)}…`
    : text;
}
...
function MemoEntryBody({
  ...
  useEffect(() => {
    ...
    const embeds = [
      ...transformed.matchAll(/!\[[^\]]*\]\(#wikiembed:([^)]+)\)/g),
...
function MemoEntryView({
  ...
  const bodyToRender = collapsible && !expanded
    ? collapsedBody(entry.body)
    : entry.body;
  ...
  return (
    <article
      className={`memo-entry ${flashing ? "memo-entry-flash" : ""}`}
    ...
    >
      <header className="memo-entry-header">
        ...
        <div className="memo-entry-actions">
          {entry.parsed && (
            <>
              <button
                ...
                disabled={busy}
                onClick={() =>
                  void run(() => onTogglePin(entry.id, !entry.pinned))}
              >
                <Pin size={13} />
              </button>
              <button
          ...
          <button
            ...
            disabled={busy}
            onClick={() => {
              if (confirm(tr("memo.deleteConfirm"))) {void run(() =>
                  onDelete(entry.id)
                );}
            }}
          >
      ...
      {entry.quote && (
        ...
        >
          ...
          {entry.anchor && (
            <span className="memo-entry-anchor">
...
export function MemoTimelinePanel({
  ...
  const lastEntryKey = displayEntries.length
    ? `${displayEntries[displayEntries.length - 1].id}`
    : "";
  useEffect(() => {
    if (lastEntryKey) {
  ...
  useEffect(() => {
    ...
    const node = listRef.current?.querySelector(
      `[data-memo-entry-id="${CSS.escape(flashEntryId)}"]`,
    );
  ...
  return (
    <aside className="memo-panel">
      ...
      <div ref={listRef} className="memo-panel-list">
        ...
        {!error && loading && (
          <div className="memo-panel-empty">{tr("common.loading")}</div>
        )}
      ...
      <footer className="memo-panel-composer">
        ...
        <div className="memo-composer-actions">
          <button
            ...
            className="widget-icon-button"
            onClick={() =>
              setComposerMode((mode) => (mode === "raw" ? "wysiwyg" : "raw"))}
