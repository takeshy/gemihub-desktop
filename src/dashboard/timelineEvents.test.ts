...
> \`Doing\` → \`Done\`
`;
  const posts = parseTimelineCalendarPosts("Dashboards/Timeline/Work/2026-07-18.md", content);
  assertEquals(posts.length, 2);
  assertEquals({ id: posts[0].id, isEvent: posts[0].isEvent, eventDate: posts[0].eventDate }, {
    id: "calendar-event-one", isEvent: true, eventDate: "2026-07-20",
  });
  assertEquals({ id: posts[1].id, isEvent: posts[1].isEvent }, { id: "kanban-history", isEvent: false });
});
...
Deno.test("memo Timeline history retains its source, quote, and body", () => {
  assertEquals(
    memoTimelineBody("workspace://Research/Paper.pdf", "Paper.pdf", "first line\nsecond line", "My note"),
