...
import { fileInventory, type DirectoryFileEntry } from "../lib/wailsBackend";
import {
  appendCalendarEvent,
...
function labels() {
  ...
  return ja ? {
    today: "今日", events: "予定", files: "作成ファイル", timeline: "Timeline", add: "予定を追加",
    empty: "この日の項目はありません。", time: "時刻（任意）", content: "内容", save: "保存", saving: "保存中…", cancel: "キャンセル",
    previous: "前の月", next: "次の月", changed: "予定の日付を変更しました。",
  } : {
    today: "Today", events: "Events", files: "Created files", timeline: "Timeline", add: "Add event",
...
export function CalendarDashboardWidget({ config, isDark, onOpenPath }: {
  ...
  const [files, setFiles] = useState<DirectoryFileEntry[]>([]);
  ...
  const load = useCallback(async () => {
    try {
      const [nextPosts, inventory] = await Promise.all([loadTimelineCalendarPosts(timelineName), fileInventory()]);
      setPosts(nextPosts);
      setFiles(inventory.filter((entry) => !entry.path.startsWith(`${TIMELINE_ROOT}/${timelineName}/`)));
      setError("");
  ...
  const filesByDay = useMemo(() => {
    const result = new Map<string, DirectoryFileEntry[]>();
    if (!showCreatedFiles) return result;
    for (const file of files) {
      const key = localDayKey(new Date(file.createdTime || file.modTime));
      const list = result.get(key) || [];
      list.push(file);
      result.set(key, list);
    }
    return result;
  }, [files, showCreatedFiles]);
  ...
  return <div className="dashboard-calendar-widget">
    ...
    {detailOpen && <div className="calendar-detail-backdrop" onMouseDown={() => setDetailOpen(false)}><section className="calendar-detail" onMouseDown={(event) => event.stopPropagation()}><button className="close" type="button" onClick={() => setDetailOpen(false)}><X size={18} /></button><header><h3>{selectedLabel}</h3><button type="button" onClick={() => setFormOpen((value) => !value)}><Plus size={14} />{copy.add}</button></header>
      ...
      {selectedEvents.length === 0 && selectedPosts.length === 0 && selectedFiles.length === 0 && !formOpen && <p className="empty">{copy.empty}</p>}
      ...
      {selectedFiles.length > 0 && <div className="calendar-detail-group"><h4><FileText size={14} />{copy.files}</h4>{selectedFiles.map((file) => <button className="calendar-file" type="button" key={file.path} onClick={() => onOpenPath(file.path)}><FileText size={14} /><span>{file.path.split("/").pop()}</span><small>{file.path}</small></button>)}</div>}
