...
Deno.test("JSON Canvasを解析して不明な接続を除外する", () => {
  const result = parseCanvas(JSON.stringify({
    nodes: [{ id: "a", type: "text", x: 1, y: 2, width: 200, height: 100, text: "hello" }],
    edges: [{ id: "ok", fromNode: "a", toNode: "a", toEnd: "arrow" }, { id: "bad", fromNode: "a", toNode: "missing" }],
  }));
...
Deno.test("不正なCanvas JSONはエラーになる", () => {
  assertStringIncludes(parseCanvas("{").error, "解析できません");
  assertStringIncludes(parseCanvas("{}").error, "nodesとedges");
});

Deno.test("ノード間の主要方向を選ぶ", () => {
  const a = { id: "a", type: "text" as const, x: 0, y: 0, width: 100, height: 100 };
